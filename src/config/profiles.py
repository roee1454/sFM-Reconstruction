"""
Quality profiles for COLMAP and OpenMVS reconstruction steps.

Provides SPEED, BALANCED, and QUALITY presets that control the trade-off
between processing time and reconstruction quality.
"""

from enum import Enum
from .settings import QUALITY_PROFILE


class QualityProfile(Enum):
    """Available quality profiles for reconstruction."""
    SPEED = "SPEED"
    BALANCED = "BALANCED"
    QUALITY = "QUALITY"


# OpenMVS parameters for each quality profile
OPENMVS_PROFILES = {
    "SPEED": {
        "DensifyPointCloud": {
...
def get_colmap_params(step_name: str) -> dict:
    ...
    profile = COLMAP_PROFILES.get(QUALITY_PROFILE, COLMAP_PROFILES["BALANCED"])
...
def get_profile_params(step_name: str) -> dict:
    ...
    profile = OPENMVS_PROFILES.get(QUALITY_PROFILE, OPENMVS_PROFILES["QUALITY"])
    return profile.get(step_name, {})
